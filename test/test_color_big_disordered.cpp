...
int main(int argc, char *argv[])
{
  ...
  double thetas_stab_x[9], thetas_stab_z[9], thetas_rot_x[19], thetas_rot_z[19];
  std::fill_n(thetas_stab_x, 9, theta_stab_x);
  std::fill_n(thetas_stab_z, 9, theta_stab_z);
  std::fill_n(thetas_rot_x, n_q, theta_rot_x);
  std::fill_n(thetas_rot_z, n_q, theta_rot_z);

  for (int i=0; i<n_itt; i++)
    {
      col_stab_x_big_disordered(thetas_stab_x, psi);
      col_stab_z_big_disordered(thetas_stab_z, psi);
      col_rot_x_big_disordered(thetas_rot_x, psi);
      col_rot_z_big_disordered(thetas_rot_z, psi);
      cout << i+1 << ":" << measure_z_big(psi) << endl;
      std::string name="./files/file_big_disordered_" + std::to_string(i) + ".txt";
      ofstream myfile;
      myfile.open (name);
      myfile << psi;
      myfile.close();
